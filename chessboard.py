...
class Board(dict):
    ...
    def shift(self, p1, p2):
        ...
        if not moves_available and self.king_in_check(piece.color):
            raise CheckMate
        if self.king_in_checkmate(p1,piece.color):
            raise CheckMate
    ...
    def pawn_promote(self):
        for pos in self.keys():
            if isinstance(self[pos], pieces.Pawn) and (pos[1] == '1' or pos[1] == '8'):
                self[pos] = pieces.create_piece('Q',self[pos].color)
                self[pos].place(self)
          
                
    def king_in_checkmate(self,p1,color):
        #Adding proper CheckMate
        if False:
            if self.king_in_check(color):
                for move in self.all_moves_available(color):
                    if not self.is_in_check_after_move(p1, list(move)): 
                        break
                    raise CheckMate
                    return True
                return False
